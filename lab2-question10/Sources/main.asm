...
LCD_RS      EQU     $80         ; LCD reset signal, pin PE7
            ...
            ORG     $3000
FNUM    FCB     $002A       ; Arbitrarily chosen random value, normally given by TA.
SNUM      FCB     $00DC       ; Arbitrarily chosen random value, normally given by TA.

mem1        DS      1            ; reserves 1 byte for variable that stores ASCII byte from Main Loop           
mem2        DS      1            ; reserves 1 byte for variable that stores ASCII byte from Main Loop
mem3        DS      1            ; reserves 1 byte for variable that stores ASCII byte from Main Loop
mem4        DS      1            ; reserves 1 byte for variable that stores ASCII byte from Main Loop
mem5        DS      1            ; reserves 1 byte for variable that stores ASCII byte from Main Loop
; code section                   
            ORG     $4000
...
iloop:      PSHA                    ;2 E-clk |
            ...
            PSHA                    ;2 E-clk |
            PULA                    ;3 E-clk | ****************************************************************************************************************
            PSHA                    ;2 E-clk | *Inserted PSHA (2 E-clk) and PULA 5 (3 E-clk) times into the loop since we need 40 E-clock cycles in each loop.* 
            PULA                    ;3 E-clk | *15 E-clock cycles already existed within the loop, so only needed to add 25 more = 5 times of PSHA and PULA.  *
            PSHA                    ;2 E-clk | *Total loops = 40 * 30 = 1200                                                                                  *
            PULA                    ;3 E-clk | *Delay = 1200 * 41.67 ns = 50000ns = 50us.                                                                     *
            PSHA                    ;2 E-clk | ****************************************************************************************************************
            PULA                    ;3 E-clk |
            PSHA                    ;2 E-clk |
            PULA                    ;3 E-clk |                                    
